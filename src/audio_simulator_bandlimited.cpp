...
AudioSimulatorBandlimited::AudioSimulatorBandlimited(SoundQueue *queue, int audio_frequency, bool audio_signed)
    : AudioSimulator(queue, audio_frequency, audio_signed) {
...
AudioSimulatorBandlimited::~AudioSimulatorBandlimited() {
    free(this->coeff_table);
    free(this->cos_table);
}

void AudioSimulatorBandlimited::note_to(uint32_t targetNotePos, uint32_t frequency, uint8_t *stream, int32_t &streamPos, int32_t streamLen) {
    ...
    if (iMax > 0) {
        ...
        uint32_t samplePos = (current_note_pos << FIXED_SHIFT_STEP);

        for (uint32_t i = 0; i < iMax; i++, samplePos += stepSize) {
            int32_t sample = 0;
            int coeff_pos = 1;
            uint64_t cos_indice = ((uint64_t) freq_real_fixed * samplePos / audio_frequency);
            int pos = freq_real_fixed;
            
            while (pos < (audio_frequency << (FIXED_SHIFT - 1)) && coeff_pos < COEFF_MAX) {
                // -(1 << 28) = 0
                // (1 << 28) = 1
            ...
            sample = ((((sample >> TRIG_SHIFT) + (1 << TRIG_SHIFT)) * (sample_max - sample_min)) >> (TRIG_SHIFT + 1)) + sample_min;
            if (sample < sample_min) sample = sample_min;
            else if (sample > sample_max) sample = sample_max;
